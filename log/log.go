...
func LoggerHandler(next echo.HandlerFunc) echo.HandlerFunc {
	return func(ctx echo.Context) error {
		tokenInfo := parseJWTTokenFromHeader(ctx)
		...
		if !strings.Contains(req.RequestURI, "healthcheck") {
			...
			req.Body = ioutil.NopCloser(bytes.NewBuffer(bodyByte))
			var body interface{}
...
func parseJWTTokenFromHeader(ctx echo.Context) (tokenInfo model.TokenInfo) {
	bearer := "Bearer "
	...
	token, _ := jwt.ParseWithClaims(jwtToken, claims, keyFunc)
	tokenInfo, _ = utils.DecodeToken(token)
	return tokenInfo
}
