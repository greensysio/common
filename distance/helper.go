...
var (
	// apiKey                   = flag.String("key", "google_api.api_key", "Must include keys..")
...
func calculateDistance(origins string, destinations string) (*maps.DistanceMatrixResponse, error) {
	flag.Parse()
	...
	if apiKey != "" {
		...
		log.Error("Please specify an API Key, or Client ID and Signature.")
	}
	if err != nil {
		return nil, err
	}
